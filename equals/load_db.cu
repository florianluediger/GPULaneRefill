...
int main()
{
    ...
    // Put search string into memory

    io::CSVReader<1> search_string_reader(SEARCH_FILENAME);
    search_string_reader.read_row(single_row);

    char *search_string;
    search_string = (char *)malloc_memory_mapped_file(sizeof(char) * single_row.length(), "mmdb/search_string");

    strcpy(search_string, single_row.c_str());
